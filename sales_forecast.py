...
def load_sales_data(file_path):
    ...
    try:
        ...
        # Инфляция из ячейки E1 (годовая, в процентах), если нет — запрашиваем у пользователя
        inflation_rate = ws['E1'].value if ws['E1'].value else float(input("Введите годовую инфляцию (в %): ")) / 100
        if len(periods) != len(prices) or len(periods) != len(volumes) or len(periods) != len(costs):
            raise ValueError("Ошибка: данные некорректны или отсутствуют.")
...
def adjust_for_inflation(value, months, inflation_rate):
    ...
    monthly_inflation = (1 + inflation_rate) ** (1 / 12) - 1  # Месячная инфляция
...
def profit_function(price, elasticity, base_price, base_volume, cost_per_unit, months, inflation_rate):
    ...
    return -(price - inflated_cost) * volume  # Отрицательное для минимизации
...
def optimize_price_volume(prices, volumes, costs, price_forecasts, volume_forecasts, periods, inflation_rate):
    ...
    avg_cost = np.mean(costs)  # Средние издержки как базовые
    optimal_results = {}
